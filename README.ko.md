...
```
Linear 이슈 (In Progress 상태)
        │
        ▼
  Orchestrator  ──폴링──▶  Linear GraphQL API
        │
        ▼  (이슈별로)
  WorkspaceManager  ──생성──▶  git worktree  {WORKSPACE_ROOT}/{이슈-키}/
        │
        ▼
  AgentRunner  ──스폰──▶  codex serve  (stdin: WORKFLOW.md 프롬프트 렌더링 결과)
        │
        ▼
...
**핵심 원칙:** Symphony는 스케줄러/러너입니다. Linear에 직접 쓰지 않습니다. 에이전트가 합니다.
...
│   │   ├── orchestrator.md          ← 폴링 루프, 상태 머신, 재시도 큐
│   │   ├── workspace-manager.md     ← 이슈별 worktree 수명주기
│   │   ├── agent-runner.md          ← JSON-RPC over stdio, SPEC §17 테스트 매트릭스
...
| 3 | **Issue Tracker Client** | Linear GraphQL 어댑터 — In Progress 이슈 조회 | `docs/specs/tracker-client.md` |
| 4 | **Orchestrator** | 폴링 루프, 상태 머신, 재시도 큐, 단일 in-memory 상태 권한 | `docs/specs/orchestrator.md` |
| 5 | **Workspace Manager** | 이슈별 `git worktree` 생성, 수명주기 훅, GC | `docs/specs/workspace-manager.md` |
| 6 | **Agent Runner** | `codex serve` 스폰, JSON-RPC over stdio, 타임아웃 강제 | `docs/specs/agent-runner.md` |
